...
ux_exact_fun = @(x,y) kx*cos(kx*x).*cos(ky*y);
uy_exact_fun = @(x,y) -ky*sin(kx*x).*sin(ky*y);
f_handle     = @(x,y) (K2^2).*u_exact_fun(x,y);     % Δ^2 u
...
[sdfxF, sdfyF] = sdf_grad_interpolants(xg, yg, sdf);
nx = sdfxF(X_bc(:,1), X_bc(:,2));
ny = sdfyF(X_bc(:,1), X_bc(:,2));
nrm = hypot(nx, ny) + 1e-15;
nx = nx ./ nrm; ny = ny ./ nrm;

%% ---------------- Centers: partition + global, then clip ------------
Nx = 20; Ny = 20;
lensX = [0.5,0.5];
lensY = [0.5,0.5];
kSigma = 5.0;
...
Phi_xx  = 2.*(ones(Nc,1)*m2) .* (2*Zx.^2 - 1) .* Phi;
Phi_yy  = 2.*(ones(Nc,1)*n2) .* (2*Zy.^2 - 1) .* Phi;
Phi_xxyy = 4.*(ones(Nc,1)*(m2.*n2)) .* (2*Zx.^2 - 1) .* (2*Zy.^2 - 1) .* Phi;
Phi_xxxx = (ones(Nc,1)*m4) .* (16*Zx.^4 - 48*Zx.^2 + 12).*Phi;
Phi_yyyy = (ones(Nc,1)*n4) .* (16*Zy.^4 - 48*Zy.^2 + 12).*Phi;

LHS_PDE = Phi_xxxx + 2*Phi_xxyy + Phi_yyyy;
RHS_PDE = f_handle(X,Y);

%% ---------------- SDF-based PDE weighting ----------------
d_pde = max(0, sdfF(X,Y));
wNear = 0.05; wFar = 1.0; delta = 0.08; p = 2.0;
...
%% ---------------- Boundary rows: Dirichlet + Neumann ----------------
Xb = X_bc(:,1); Yb = X_bc(:,2);

Zx_b = Xb.*m.' + alpha.'; 
Zy_b = Yb.*n.' + beta.'; 
Z2_b = Zx_b.^2 + Zy_b.^2;
Phib = exp(-min(Z2_b,700));

Phib_x = -2.*(ones(Nb,1)*m.').*Zx_b.*Phib;
Phib_y = -2.*(ones(Nb,1)*n.').*Zy_b.*Phib;
Phi_n  = nx.*Phib_x + ny.*Phib_y;

Ub   = u_exact_fun(Xb,Yb);
Un_b = nx.*ux_exact_fun(Xb,Yb) + ny.*uy_exact_fun(Xb,Yb);

% wB1 = 60; 
wB1 = 10; 
wB2 = 40;

LHS_BC1 = wB1*Phib;      RHS_BC1 = wB1*Ub;
LHS_BC2 = wB2*Phi_n;     RHS_BC2 = wB2*Un_b;

%% ---------------- Assemble & Solve ----------------
H = [LHS_PDE; LHS_BC1; LHS_BC2];
b = [RHS_PDE; RHS_BC1; RHS_BC2];
...
%% ---------------- Plots ----------------
figure('Color','w','Position',[70 70 1800 740]);
...
plot(X_bc(:,2),X_bc(:,1),'k.','MarkerSize',6); hold off;

subplot(2,3,2);
scatter(X_pde(:,1),X_pde(:,2),20,wPDE_vec,'filled'); axis equal tight; colorbar;
title('PDE row weight $w_{\mathrm{PDE}}(x,y)$');
xlabel('$x$'); ylabel('$y$'); grid on;

subplot(2,3,3);
plot(X_bc(:,1),X_bc(:,2),'k.'); axis equal tight; grid on;
title(sprintf('Boundary points ($N_b=%d$)', Nb_side));
xlabel('$x$'); ylabel('$y$');

subplot(2,3,4);
surf(XX,YY,U_pred,'EdgeColor','none'); view(2); colorbar; grid on;
title('KAPI--ELM solution');
xlabel('$x$'); ylabel('$y$');

subplot(2,3,5);
surf(XX,YY,U_exact,'EdgeColor','none'); view(2); colorbar; grid on;
title('Exact solution (masked)');
xlabel('$x$'); ylabel('$y$');
...
%% ---- Save figure ----
saveas(gcf, 'TC_06_Irregular_Biharmonic.png');
disp('Figure saved: TC_06_Irregular_Biharmonic.png');
...
exportgraphics(fig_error, 'TC_06_Irregular_Biharmonic_error.png', 'Resolution', 300);
...
% end
function [xg, yg, sdf, inMask] = build_irregular_domain_SDF(N)
% Irregular "flower" mask on [0,1]^2 and a consistent signed distance field.
% Convention: sdf > 0 inside, sdf = 0 on boundary, sdf < 0 outside.
    ...
    inMask = (R <= Rb);          % logical mask (true inside)

    % --- Signed distance in physical units (grid spacing h) ---
    % bwdist returns Euclidean distance in *pixels*
    h   = 1/(N-1);
    Din  = bwdist(~inMask);      % distance to outside (zero outside)
    Dout = bwdist(inMask);       % distance to inside  (zero inside)
    sdf  = (Din - Dout) * h;     % => positive inside, negative outside

    % enforce types (contourc wants double)
    xg  = double(xg);
    yg  = double(yg);
    sdf = double(sdf);
    inMask = logical(inMask);
...
function [sdfxF, sdfyF] = sdf_grad_interpolants(xg, yg, sdf)
% Build interpolants for the gradient of SDF using centered differences.
    [SDFX, SDFY] = gradient(sdf, xg, yg);  % MATLAB gradient uses xg,yg spacing
    sdfxF = griddedInterpolant({xg, yg}, SDFX.', 'linear', 'nearest');
    sdfyF = griddedInterpolant({xg, yg}, SDFY.', 'linear', 'nearest');
end
